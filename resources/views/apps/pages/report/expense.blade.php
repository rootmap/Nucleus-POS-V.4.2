...
<section id="form-action-layouts">
	                            ...
	                            <div class="row">
	                                ...
	                                <div class="col-md-12">
	                                    ...
	                                    <div class="input-group" style="margin-top:32px;">
											...
											<a href="javascript:void(0);" data-url="{{url('expense/voucher/excel/report')}}" class="btn btn-green btn-darken-2 mr-1 change-action" @if($userguideInit==1) data-step="3" data-intro="If you click this button then it will generate excel file." @endif>
	...
	@if($userguideInit==1) data-step="4" data-intro="If you click this button then it will generate pdf file." @endif>
											...
											<a href="{{url('expense/voucher/report')}}" style="margin-left: 5px;" class="btn btn-green btn-accent-4 mr-1" @if($userguideInit==1) data-step="5" data-intro="if you want clear all information then click the reset button." @endif>
												<i class="icon-refresh"></i> Reset
											</a>
											<a href="{{url('expense/voucher')}}" class="btn btn-green btn-darken-4 mr-1" @if($userguideInit==1) data-step="6" data-intro="By clicking this button you can create Expense Voucher." @endif>
		...
		<div class="card">
			...
			<div class="card-body collapse in">
				<div class="table-responsive">
					<table class="table table-striped table-bordered" id="expense_report">
						<thead>
							<tr>
								<th>Voucher ID</th>
								<th>Expense Head</th>
						...
						<tbody>
							...
							@if(isset($invoice))
								...
								<tr>
	                                ...
	                                <td>{{formatDate($inv->expense_date)}}</td>
   ...
   <script>
    ...
    $(document).ready(function(e){
        ...
        @if(!empty($expense_id) || !empty($start_date) || !empty($end_date))
            @if(isset($invoice))
                @if(count($invoice)>0)
                    $('#expense_report').DataTable({"aLengthMenu": [[25, 50, 100, 500,999999999999999999], [25, 50, 100, 500, "All"]]});
                @endif
            @endif
        @else
        ...
        $('#expense_report').dataTable({
            ...
            "ajax":{
                url :"{{url('expense/voucher/report/json')}}",
                headers: {
                ...
                complete:function(data){
                    ...
                    $.each(totalData.data,function(key,row){
                        ...
                        strHTML+='      <td>'+row.expense_name+'</td>';
                        strHTML+='      <td>'+formatDate(replaceNull(row.expense_date))+'</td>';
                        strHTML+='      <td>'+replaceNull(row.expense_description)+'</td>';
                        strHTML+='      <td>'+replaceNull(row.expense_amount)+'</td>';
                ...
                error: function(){
                  $("#expense_report_processing").css("display","none");
                }
            }
        });
